...
public class PointsOfInterestSubLayerTreeView : TreeView
{
	...
	protected override TreeViewItem BuildRoot()
	{
		...
		if (Layers != null)
		{
			...
			{
				var name = Layers.GetArrayElementAtIndex(i).FindPropertyRelative("properties.name").stringValue;
	...
	protected override void RenameEnded(RenameEndedArgs args)
	{
		...
		layer.FindPropertyRelative("properties.name").stringValue = string.IsNullOrEmpty(args.newName.Trim()) ? args.originalName : args.newName;
	}
	...
	protected override void RowGUI(RowGUIArgs args)
	{
		...
		item.FindPropertyRelative("isActive").boolValue = EditorGUI.Toggle(toggleRect, item.FindPropertyRelative("coreOptions.isActive").boolValue);
		if (EditorGUI.EndChangeCheck())
		{
			hasChanged = true;
		}
		args.item.displayName = item.FindPropertyRelative("properties.name").stringValue;
		base.RowGUI(args);
	}
}
