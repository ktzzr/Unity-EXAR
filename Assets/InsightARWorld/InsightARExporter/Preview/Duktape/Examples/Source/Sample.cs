...
public class Sample : MonoBehaviour, IDuktapeListener
{
    ...
    {
        Debug.Log("duktape loaded " + Time.time); ;
        tests();
        vm.AddSearchPath("Assets/InsightARWorld/Duktape/Examples/Scripts/out");
        do
        {
    ...
    private void RunScript(DuktapeVM vm)
    {
        if (jsBytecode)
        {
            ...
            else
            {
                ...
                System.IO.File.WriteAllBytes("Assets/InsightARWorld/Duktape/Examples/Scripts/out/" + launchScript + ".bytes", bytecode);
                vm.EvalMain(launchScript, bytecode);
            }
        }
        else
    ...
    private void tests()
    {
        ...
        DateTime.Now.ToFileTime();
        {
            ...
            SampleNamespace.SampleClass.WriteLog(string.Format("\nc#/DoNothing321 {0}", (DateTime.Now - start).TotalSeconds));
        }
        {
            var start = DateTime.Now;
            for (var i = 1; i < 1000000; i++)
            ...
            SampleNamespace.SampleClass.WriteLog(string.Format("c#/DoNothing123 {0}", (DateTime.Now - start).TotalSeconds));
        }
        {
            var start = DateTime.Now;
    ...
    void Awake()
    {
        ...
        vm.Initialize(this);
        Debug.Log("duktape start " + Time.time) ;
    }
