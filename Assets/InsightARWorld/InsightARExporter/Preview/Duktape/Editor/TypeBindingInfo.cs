...
namespace Duktape
{
    ...
    public class TypeBindingInfo
    {
        ...
        public string GetFileName()
        {
            ...
            //fix by wy 修改导出CS函数名称，UnityEngine->Insight
            var filename = "";
            if (BindingManager.isInsightNameSpace)
            {
                string[] typeFullName = type.FullName.Split(new char[] { '.', '<', '>' });
                if (type.FullName.Contains("UnityEngine"))
                {
                    filename = BindingManager.STRINSIGHT +"_"+ typeFullName[typeFullName.Length - 1];
                }
                else
                {
                    filename = type.FullName.Replace(".", "_").Replace("<", "_").Replace(">", "_").Replace("`", "_");
                }
            }
            else
            {
            ...
            //add wrap
            if (BindingManager.isFileNameWrapSuffix) filename += BindingManager.STRWRAPPER;
            return filename;
        }
