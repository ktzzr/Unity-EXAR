...
namespace Duktape
{
    ...
    public class RegFuncNamespaceCodeGen : IDisposable
    {
        ...
        public RegFuncNamespaceCodeGen(CodeGenerator cg, TypeBindingInfo bindingInfo)
        {
            ...
            {
                //edit by wy UnityEngine->Insight namespace
                string[] split_ns ;
                if (BindingManager.isInsightNameSpace)
                {
                    if (bindingInfo.jsNamespace.StartsWith("UnityEngine"))
                    {
                        split_ns = new string[] { BindingManager.STRINSIGHT };
                    }
                    else
                    {
                        split_ns = bindingInfo.jsNamespace.Split('.');
                    }
                }
                else
                {
    ...
    public class TypeCodeGen : IDisposable
    {
        ...
        public TypeCodeGen(CodeGenerator cg, TypeBindingInfo bindingInfo)
        {
            ...
            string[] bindingName = bindingInfo.name.Split('_');
            if (bindingName.Length == 4)
            {
                string newBindingName = bindingName[0] + "_" + bindingName[1] + "_" + bindingName[3];
                this.cg.cs.AppendLine("public class {0} : {1} {{", newBindingName, typeof(DuktapeBinding).Name);
            }
            else {
