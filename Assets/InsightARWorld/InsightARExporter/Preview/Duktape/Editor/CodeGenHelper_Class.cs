                ...
                using (new RegFuncCodeGen(cg))
                {
                    using (new RegFuncNamespaceCodeGen(cg, bindingInfo))
                    {
                        ...
                        foreach (var kv in bindingInfo.methods)
                        {
                            ...
                            //fix by wy，首字母小写，只能字母名称通过
                            if (BindingManager.isMethodNameStartCharLowerCase)
                            {
                                if (!Regex.IsMatch(regName, @"^[A-Za-z0-9]+$")) continue;
                                string lowerRegName = regName[0].ToString().ToLower() + regName.Substring(1);
                                cg.cs.AppendLine("duk_add_method(ctx, \"{0}\", {1}, {2});", lowerRegName, funcName, bStatic ? -2 : -1);
                            }
                            else
                            {
                        ...
                        foreach (var kv in bindingInfo.staticMethods)
                        {
                            ...
                            /*    if (BindingManager.isMethodNameStartCharLowerCase)
                                {
                                    if (!Regex.IsMatch(regName, @"^[A-Za-z]+$")) continue;
                                    string lowerRegName = regName[0].ToString().ToLower() + regName.Substring(1);
                                    cg.cs.AppendLine("duk_add_method(ctx, \"{0}\", {1}, {2});", lowerRegName, funcName, bStatic ? -2 : -1);
                                }
                                else
