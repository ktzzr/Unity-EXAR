...
namespace Duktape
{
    ...
    public class DuktapeObject : DuktapeValue
    {
        ...
        public DuktapeFunction GetMethod(string name)
        {
            DuktapeFunction func = null;
            if (_methodCache != null)
            {
                if (_methodCache.ContainsKey(name))
                {
                   
                    return _methodCache[name];
                }
            }
            return func;
        }

        public DuktapeFunction AddMethod(string name)
        {
                ...
                if (ctx != IntPtr.Zero)
                {
                    if (DuktapeDLL.duk_get_prop_string(ctx, -1, name)){
                        if (DuktapeDLL.duk_is_function(ctx, -1))
                        {
                            ...
                            method = new DuktapeFunction(ctx, 0, ptr);
                        }
                        else
                        {
                        }
        ...
        public bool InvokeMemberWithBooleanReturn(string name)
        {
            ...
            if (member != null)
            {
                ...
                if (ctx != IntPtr.Zero)
                {
                    ...
                    {
                        DuktapeAux.PrintError(ctx, -1, "name");
                        DuktapeDLL.duk_pop(ctx);
