...
public static class XcodePostBuild
{
    ...
    /// Sample value: "../xcode"
    /// </summary>
	private const string XcodeProjectRoot = "../oasis2.0_ios_unity";
    ...
    /// </summary>
    private const string XcodeProjectName = "UnityExporter";

    private const string XcodeProjectFileName = "Unity-iPhone";
    ...
    private const string PbxFilePath = XcodeProjectFileName + ".xcodeproj/project.pbxproj";

    private const string BackupExtension = ".bak";

    private const string TouchedMarker = "wy";
    ...
    public static void OnPostBuild(BuildTarget target, string pathToBuiltProject)
    {
      ...
      //  UpdateUnityProjectFiles(pathToBuiltProject);
		...
		Debug.Log ("-- XcodePostBuild.cs process done --");
    }
    ...
    private static void PatchUnityNativeCode(string pathToBuiltProject)
    {
      ...
      EditUnityAppControllerMM(Path.Combine(pathToBuiltProject, "Classes/Unity/UnityWebRequest.mm"));
    }
    ...
    private static void EditUnityAppControllerMM(string path)
    {
        ...
        //                @" #import <OasisUnityPlugin/OasisUnityPlugin-Swift.h>",
        //                @" #import <Firebase.h>",
        //            };
        //        }
        //    }
        ...
        //var inScope1 = false;
        //var markerDetected1 = false;

        //EditCodeFile(path, line =>
        //{
        //    inScope1 |= line.Contains("- (BOOL)application:(UIApplication*)application");
        //    markerDetected1 |= inScope1 && line.Contains(TouchedMarker);

        //     if (inScope1 && line.Trim() == "_unityView      = [self createUnityView];")
        //    {
        //        inScope1 = false;
        ...
        //var inScope2 = false;
        //var markerDetected2 = false;

        //EditCodeFile(path, line =>
        //{
        //    inScope2 |= line.Contains("::printf(\"-> applicationDidEnterBackground()\\n\");");
        //    markerDetected2 |= inScope2 && line.Contains(TouchedMarker);

        //    if (inScope2 && line.Trim() == "}")
        //    {
        //        inScope2 = false;
        ...
        //                @" if (_unityView)",
        //                @"  {",
        //                @"    [_unityView addSubview:[[OSMapPluginView alloc] initWithFrame:[UIScreen mainScreen].bounds]];",
        //                @"  }",
        //                @"}",
        //            };
        //        }
        ...
        var inScope3 = false;
        var markerDetected3 = false;

        EditCodeFile(path, line =>
        {
            inScope3 |= line.Contains("webOperationQueue.name = @\"com.unity3d.WebOperationQueue\";");
            markerDetected3 |= inScope3 && line.Contains(TouchedMarker);
    ...
    private static void EditDeviceSettingsMM(string path)
    {
        ...
        EditCodeFile(path, line =>
        {
            ...
            if (inScope && !markerDetected)
            {
                ...
                if (level > 0 && line.Trim().StartsWith("#endif"))
                {
                    return new string[]
                    {
                        line,
    ...
    private static void EditUnityViewControllerViewHandlingMM(string path)
    {
        var inScope0 = false;
        var markerDetected0 = false;
        ...
        var inScope4 = false;
        var markerDetected4 = false;

        EditCodeFile(path, line =>
        {
            //解决自动旋转屏幕问题，引入头文件
            inScope0 |= line.Trim() == "#include <utility>";
            markerDetected0 |= inScope0 && line.Contains(TouchedMarker);
            ...
            if (inScope0)
            {
                ...
                else
                {
                    return new string[]
                    {
                        line,
                        "// Modified by " + TouchedMarker,
                        "// include auto rotate header file",
                      @"#include ""AppBridge.h"""
                    };
                }
            }

            //解决自动旋转屏幕问题，发送消息，设置自动旋转不生效
            inScope1 |= line.Trim() == "- (void)transitionToViewController:(UIViewController*)vc";
            markerDetected1 |= inScope1 && line.Contains(TouchedMarker);

            if (inScope1 &&  line.Trim() == "[self didTransitionToViewController: vc fromViewController: _rootController];")
            {
                inScope1 = false;
                ...
                else
                {
                    return new string[]
                    {
                       ...
                       @"    [[NSNotificationCenter defaultCenter] postNotificationName: @""kShowUnitySubView"" object:self];",
                       @"    shouldAutorotate(false);"
                    };
                }
            }

            //解决unity window 和 iOS window 冲突问题，设置unity uiwindow hidden true
            inScope2 |= line.Trim() == "- (void)showGameUI";
            markerDetected2 |= inScope2 && line.Contains(TouchedMarker);

            if (inScope2 && line.Trim() == "[_window bringSubviewToFront: _rootView];")
            {
                inScope2 = false;
                ...
                else
                {
                    return new string[]
                    {
                        line,
                        "    // Modified by " + TouchedMarker,
                        "    // hidden unity uiwindow.",
                       "    _window.hidden = YES;",
                    };
                }
            }

            //解决unity window 和 iOS window 冲突问题，注释unity uiwindow hidden = NO
            inScope3 |= line.Trim() == "- (void)transitionToViewController:(UIViewController*)vc";
            markerDetected3 |= inScope3 && line.Contains(TouchedMarker);

            if (inScope3 && line.Trim() == "_window.hidden = NO;")
            {
                inScope3 = false;
            ...
            //解决unity window 和 iOS window 冲突问题，注释makekeyandvisible
            inScope4 |= line.Trim() == "- (void)transitionToViewController:(UIViewController*)vc";
            markerDetected4 |= inScope4 && line.Contains(TouchedMarker);

            if (inScope4 && line.Trim() == "[_window makeKeyAndVisible];")
            {
                inScope4 = false;
