...
{
    #region params
    private const string TAG = "FileDownloadThread";
    ...
    public void Run()
    {
        ...
        if (mTryCount > 1)
        {
            InsightDebug.Log(TAG, "Retry Count " + (mTryCount - 1));
        }
    ...
    private bool CheckMd5()
    {
        ...
        if (md5 != downloadTask.MD5)
        {
            ...
            InsightDebug.Log(TAG, "文件MD5校验出错：" + downloadTask.FileName);
    ...
    public bool Download()
    {
        ...
        {
            //文件已存在，跳过
            InsightDebug.Log(TAG, "File Exists " + downloadTask.SavePath);
            mCurSize = downloadTask.Size;
            return true;
        }
        else if (File.Exists(tempFile))
        {
            InsightDebug.Log(TAG, "Temp File Exists " + tempFile);
        ...
        {
            InsightDebug.Log(TAG, "Download File " + tempFile);
        ...
        catch (Exception ex)
        {
            ...
            InsightDebug.Log(TAG, "Catch Download Error：" + ex.Message);
            mState = FileDownloadState.Error;
        ...
        finally
        {
            ...
            InsightDebug.Log(TAG, "Download Catch Finally ");
        }
    ...
    private long GetWebFileSize(string url)
    {
        ...
        {
            InsightDebug.Log(TAG, "获取文件长度出错：" + e.Message);
            mState = FileDownloadState.Error;
