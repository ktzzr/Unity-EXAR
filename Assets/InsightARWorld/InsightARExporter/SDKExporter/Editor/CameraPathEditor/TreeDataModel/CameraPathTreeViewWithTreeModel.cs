...
internal class CameraPathTreeViewWithTreeModel<T> : TreeView where T : TreeElement
{
	...
	public CameraPathTreeViewWithTreeModel(TreeViewState state, TreeModel<T> model) : base(state)
	{
		Init(model);
	}

	public CameraPathTreeViewWithTreeModel(TreeViewState state, MultiColumnHeader multiColumnHeader, TreeModel<T> model)
	...
	void AddChildrenRecursive(T parent, int depth, IList<TreeViewItem> newRows)
	{
		foreach (T child in parent.children)
		{
			...
			item.icon = EditorGUIUtility.FindTexture("AudioSource Icon");
			newRows.Add(item);
