...
void main ()
{
  ...
  tmpvar_26 = ((xlv_TEXCOORD0 * _SpecGlossMap_ST.xy) + _SpecGlossMap_ST.zw);
  lowp vec4 tmpvar_27 = vec4(0.0);
  tmpvar_27 = texture2D (_SpecGlossMap, tmpvar_26);
  tex2DNode36_19 = tmpvar_27;
  tmpvar_15 = tex2DNode36_19.xyz;
  tmpvar_17 = (tex2DNode36_19.w * _GlossMapScale);
  ...
  tmpvar_31 = dot (xlv_TEXCOORD1.xyz, tmpvar_16);
  worldN_5.x = tmpvar_31;
  highp float tmpvar_32 = 0.0;
  tmpvar_32 = dot (xlv_TEXCOORD2.xyz, tmpvar_16);
  worldN_5.y = tmpvar_32;
  highp float tmpvar_33 = 0.0;
  tmpvar_33 = dot (xlv_TEXCOORD3.xyz, tmpvar_16);
  ...
  albedo_49 = tmpvar_14;
  mediump vec3 specColor_50 = vec3(0.0);
  specColor_50 = tmpvar_15;
  mediump float oneMinusReflectivity_51 = 0.0;
  ...
  mediump vec3 specColor_55 = vec3(0.0);
  specColor_55 = tmpvar_15;
  mediump vec3 normal_56 = vec3(0.0);
  normal_56 = tmpvar_47;
  mediump vec3 tmpvar_57 = vec3(0.0);
  mediump vec3 inVec_58 = vec3(0.0);
  inVec_58 = (tmpvar_4 + viewDir_45);
  tmpvar_57 = (inVec_58 * inversesqrt(max (0.001, 
    dot (inVec_58, inVec_58)
  )));
  mediump float tmpvar_59 = 0.0;
  tmpvar_59 = clamp (dot (normal_56, tmpvar_57), 0.0, 1.0);
  mediump float tmpvar_60 = 0.0;
  tmpvar_60 = (1.0 - tmpvar_17);
  mediump float tmpvar_61 = 0.0;
  tmpvar_61 = (tmpvar_60 * tmpvar_60);
  mediump float x_62 = 0.0;
  x_62 = (1.0 - clamp (dot (normal_56, viewDir_45), 0.0, 1.0));
  mediump vec4 tmpvar_63 = vec4(0.0);
  tmpvar_63.w = 1.0;
  tmpvar_63.xyz = (((
    ((diffColor_54 + ((tmpvar_61 / 
      ((max (0.32, clamp (
        dot (tmpvar_4, tmpvar_57)
      , 0.0, 1.0)) * (1.5 + tmpvar_61)) * (((tmpvar_59 * tmpvar_59) * (
        (tmpvar_61 * tmpvar_61)
       - 1.0)) + 1.00001))
    ) * specColor_55)) * tmpvar_3)
   * 
   ...
   * 
    ...
    clamp ((tmpvar_17 + (1.0 - oneMinusReflectivity_51)), 0.0, 1.0)
  ), vec3(
    ((x_62 * x_62) * (x_62 * x_62))
  ))));
